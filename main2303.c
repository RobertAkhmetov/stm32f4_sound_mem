...
//	HAL_TIM_Base_Start_IT(&htim2);
	...
	*/
	...
	int maxBuff=32768;
	uint8_t picture[maxBuff]; //фрагмент картинки, который мы запишем в память
	int blockNumber = 0;
		for (int indexElemInArr = 0; indexElemInArr< maxBuff; indexElemInArr++)
		{
	...
	---------------------------------------    С ВНЕШНЕЙ ПАМЯТИ ------------------------------*/	
		uint8_t picture1[32768];
		AT45_ReadMainMemoryPage( page, offset);
		//for(int i=0;i<32768;i++)
		//{
			//AT45_ReadMainMemoryPage( page, offset); //может не туда поставил
			HAL_SPI_Receive(&hspi2, &byte, 1, 0);	picture1[0] = byte;
		//}

		//ILI9341_Draw_Image((uint8_t*)picture1);
		HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_SET); // зеленый вкл
